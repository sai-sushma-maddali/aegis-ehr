...
export default function AnswerCard({ result, isRunning }) {
  ...
  if (result.overall_status === "CHAT" || result.searched === false) {
    return (
      <div className="card">
        <div className="answer-prose">{result.answer}</div>
      </div>
    );
  }
  ...
  return (
    <div className="card">
      <div className="answer-header">
        <div>
          <div className="answer-patient">
            {result.patient_name || "Chart search"}
            {result.patient_id ? (
              <span className="answer-mrn">MRN {result.patient_id}</span>
            ) : null}
          </div>
        </div>
      ...
      {result.answer ? (
        ...
        <div className="empty-state">No matching passage was found in the charts.</div>
      )}
      ...
      <EvidenceTable result={result} isRunning={false} />
    </div>
  );
}
